...
import java.sql.Connection;//Kết nối với csdl
import java.sql.DriverManager;//Qlds các Drivers
import java.sql.ResultSet;//Bảng tập dl sau khi tt
import java.sql.SQLException;//Truy vấn thông tin về lỗi
import java.sql.Statement;//Thực thi 1 câu lệnh SQL tĩnh
...
public class ConnectSQL {
    ...
    //Lấy ra thông tin bàn theo mã bàn
    ...
    public ArrayList<ThucDon> GetThucDon(String ma){
        ...
        try{
            Statement st = cn.createStatement();
            ResultSet rs = st.executeQuery(sql);
            arrThucDon = new ArrayList<>();//arrThucDon = new ArrayList<ThucDon>();
            while(rs.next()){
                ThucDon td = new ThucDon(rs.getString(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5));
                arrThucDon.add(td);
            }
...
//            arrtd = new ArrayList<TaiKhoan>();
    ...
    //Lấy ra tài khoản theo name và pass
